...
class PDF extends TCPDF {
    ...
    public function Header() {
        ...
        $this->Ln(13);
        $this->Cell(0, 0, 'Inventario actual', 0, false, 'C', 0, '', 0, false, 'M', 'M');
...
$pdf->setTitle('Inventario actual');
$pdf->setSubject('Inventario actual');
$pdf->setKeywords('Inventario actual, PDF, INSAFORP, UNIVO');
...
if($Istatus == "activo"){
    $respuesta = ModeloReportes::mdlMostrarInventarioActual(1);
}if($Istatus == "inactivo"){
    $respuesta = ModeloReportes::mdlMostrarInventarioActual(2);
}
...
<table cellspacing="0" cellpadding="1" border="0" style="border-color:gray;">
    <tr style="background-color:#1f4271;color:#FFF; padding: 3px;">
        ...
        <td style="width: 130rem">Nombre</td>
		<td style="width: 40rem">Precio</td>
        <td>Categoria</td>
        <td>No. Contrato</td>
        <td>No. Oferta</td>
        <td>Fecha</td>
    ...
    foreach($respuesta as $key => $value) {
            $html .= '<tr>
                <td border="0.35" style="text-align: center;">'.$value["cantidadactual"].'</td>
                ...
                <td border="0.35">'.$value["numero_contrato"].'</td>
                <td border="0.35" style="text-align: center;">'.$value["numero_oferta_compra"].'</td>
